...
Player* createPlayer(SDL_Renderer *pRederere,char playerName[NAME], int window_h, int window_w){
    ...
    pPlayer->sprits[1] = (SDL_Rect){180,280,32,64}; // gå fram 1
    ...
    pPlayer->sprits[7] = (SDL_Rect){310,280,32,64}; //gå back 1
    ...
    pPlayer->sprits[13] = (SDL_Rect){50,280,32,64}; // gå rhigt 1
...
void rednerPlayer(SDL_Renderer *pRenderer, Player *pPlayer, SDL_Point Ofset){
    ...
    if(pPlayer->deltaTime>= delay){
        if(Ofset.y<0){ // upp
            ...
            pPlayer->indexOfSprits++;
            if(pPlayer->indexOfSprits>12 || pPlayer->indexOfSprits<6){
                pPlayer->indexOfSprits = 7; 
            }
        }
        if(Ofset.x<0){ //rhigt 
            pPlayer->indexOfSprits++;
            if(pPlayer->indexOfSprits>18 || pPlayer->indexOfSprits<12){
        ...
        if(Ofset.x == 0 && Ofset.y == 0){
            if(pPlayer->indexOfSprits<= 5){
                pPlayer->indexOfSprits = 0;
            }else if (pPlayer->indexOfSprits <= 11){
                pPlayer->indexOfSprits = 6;
            }else if (pPlayer->indexOfSprits <= 17){
                pPlayer->indexOfSprits = 12;
            }
        }
        pPlayer->deltaTime = 0;
    }
